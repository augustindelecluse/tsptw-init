...
public class Distance extends AbstractConstraint {
     ...
     * @param seqVar sequence whose nodes need to be visited
     * @param transition transition from one node to another
     * @param distance distance of the sequence
     */
    public Distance(OldSeqVar seqVar, int[][] transition, IntVar distance) {
        super(seqVar.getSolver());
        this.transition = transition;
        this.distance = distance;
        this.seq = seqVar;
        possible = new int[seqVar.nNode()];
        insertions = new int[seqVar.nNode() + 2];
    }
