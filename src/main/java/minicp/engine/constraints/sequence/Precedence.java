...
public class Precedence extends AbstractConstraint {
     ...
     */
    public Precedence(OldSeqVar seq, int... order) {
     ...
     */
    public Precedence(OldSeqVar seq, boolean mustAppear, int... order) {
        super(seq.getSolver());
        this.seq = seq;
    ...
    public void post() {
                ...
                if (seq.isExcluded(node)) { // a node is excluded, exclude the remaining one
                    for (int exclude: order) {
                        if (seq.isMember(exclude))
                            throw INCONSISTENCY; // a node must be excluded but is already a member
                        else
                            seq.exclude(exclude);
                    }
        ...
        for (int i = 0; i < order.length ; ++i)
            for (int j = i+1; j < order.length; ++j)
                seq.removePredInsert(order[j], order[i]); // order[i] cannot have order[i+1...n] as predecessor
        propagate();
        if (isActive()) {
            seq.propagateOnFix(this);
            if (!useIncremental) {
    ...
    public void propagate() {
        ...
        // filter member insertions that are invalid
        ...
        for (int node: order) {
            if (seq.isMember(node)) {
                ...
                int size = seq.fillMemberPredInsert(node, insertions); // retrieve the insertions
                for (i = 0 ; i < size ; ++i) {
                    if (seqPosition.get(insertions[i]) < predPos) // if the insert is before the pred node in order, remove it
                        seq.removePredInsert(insertions[i], node);
                }
            }
        }
